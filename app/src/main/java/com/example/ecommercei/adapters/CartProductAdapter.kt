...
import com.example.ecommercei.databinding.CartProductItemBinding
...
class CartProductAdapter: RecyclerView.Adapter<CartProductAdapter.CartProductsViewHolder>() {
    inner class CartProductsViewHolder( val binding: CartProductItemBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(cartProduct: CartProduct){
            binding.apply {
                ...
                productCardNameTv.text = cartProduct.product.name
                tvCartProductQuantity.text = cartProduct.quantity.toString()

                val priceAfterPercentage = cartProduct.product.offerPercentage
                    .getProductPrice(cartProduct.product.price)
                productCardPriceTv.text = "$ ${String.format("%.2f",priceAfterPercentage)}"

                imageCardProductColor.setImageDrawable(ColorDrawable(cartProduct.selectedColor?: Color.TRANSPARENT))
                tvCartProductSize.text = cartProduct.selectedSize ?: "".also {
                    imageCardProductSize.setImageDrawable(ColorDrawable(Color.TRANSPARENT))
                }
    ...
    override fun onBindViewHolder(holder: CartProductsViewHolder, position: Int) {
        ...
        holder.binding.imageCardProduct.setOnClickListener {
